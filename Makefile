...
install_app:
	$(VAGRANT_CMD) ssh -c 'sudo /opt/splunk/bin/splunk install app /vagrant/splunk_app_campfire.spl -update true -auth admin:ampledata'
...
setup_app:
	curl -k -u admin:ampledata https://localhost:4179/servicesNS/nobody/splunk_app_campfire/apps/local/splunk_app_campfire/setup -d /campfire/api/api/subdomain=$(CAMPFIRE_SUBDOMAIN) -d /campfire/api/api/room_name=$(CAMPFIRE_ROOM_NAME) -d /campfire/api/api/auth_token=$(CAMPFIRE_AUTH_TOKEN)
